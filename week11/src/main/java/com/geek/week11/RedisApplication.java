...
public class RedisApplication {
    ...
    public static void countDown() {
        ...
        // 模拟分布式，新建15个线程
        ExecutorService controllerThreadPool = Executors.newFixedThreadPool(15);
        for (int i = 0; i < 15; i++) {
            controllerThreadPool.execute(() -> {
                // 连接redis
                Jedis jedis = new Jedis("localhost", 6379);
    ...
    public static class Subscriber extends JedisPubSub {
        ...
        public void onMessage(String channel, String message) {
            System.out.println(String.format("从频道（channel） %s接收 redis 发布的消息, message %s", channel, message));
        }

        public void onSubscribe(String channel, int subscribedChannels) {
            System.out.println(String.format("订阅 redis 频道channel：channel %s 成功, subscribedChannels %d",
                    channel, subscribedChannels));
        }

        public void onUnsubscribe(String channel, int subscribedChannels) {
            System.out.println(String.format("不订阅redis 频道channel：channel %s 成功, subscribedChannels %d",
                    channel, subscribedChannels));
        }
    }
}
